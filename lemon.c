...
PRIVATE int translate_code(struct lemon *lemp, struct rule *rp){
  ...
  if( lhsdirect ){
    sprintf(zLhs, "yystack(%d).minor.yy%d",1-rp->nrhs,rp->lhs->dtnum);
  }else{
    rc = 1;
          ...
          if( rp->rhsalias[i] && strcmp(cp,rp->rhsalias[i])==0 ){
            if( i==0 && dontUseRhs0 ){
              ...
              ** the token number of X, not the value of X */
              append_str("yystack(%d).major",-1,i-rp->nrhs+1,0);
            }else{
              struct symbol *sp = rp->rhs[i];
              int dtnum;
              if( sp->type==MULTITERMINAL ){
              ...
              append_str("yystack(%d).minor.yy%d()",0,i-rp->nrhs+1, dtnum);
            }
            cp = xp;
            used[i] = 1;
            break;
  ...
  ** saved LHS value now. */
  if( lhsdirect==0 ){
    append_str("  yystack(%d).minor.yy%d(", 0, 1-rp->nrhs, rp->lhs->dtnum);
    append_str(zLhs, 0, 0, 0);
    append_str("());\n", 0, 0, 0);
  }
...
void ReportTable(
  ...
  if( lemp->ctx && lemp->ctx[0] ){
    ...
    fprintf(out,"#define %sCTX_FETCH %s=this.%s;\n",
                 name,lemp->ctx,&lemp->ctx[i]);  lineno++;
    fprintf(out,"#define %sCTX_STORE this.%s=%s;\n",
  ...
  */
  for(i=0, rp=lemp->rule; rp; rp=rp->next, i++){
    fprintf(out,"  new ruleInfoEntry(%4d, %4d), /* (%d) ",rp->lhs->index,-rp->nrhs,i);
    rule_print(out, rp);
    fprintf(out," */\n"); lineno++;
  }
  tplt_xfer(lemp->name,in,out,&lineno);
